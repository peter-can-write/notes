...
Gesamtdauer ($a \rightarrow t$) von $T$. Die Knoten $s$ und $t$ liegen dabei $d$
...
Zeit. Danach wird die eigentliche Nachricht versandt. Die serialisierten Daten
werden dabei von Knoten zu Knoten ohne Verzoegerung versandt. Also brauchen wir
...
Signal, was wiederum $t_p$ Sekunden braucht, um von $t$ nach $s$ zu kommen.

Insgesamt haben wir also: $T_{LV} = 2t_p + t_s + 2t_p = t_s + 4t_p = \frac{L}{r}
+ 4\frac{d}{\nu c}$.
...
Vermittlungsentscheidungen getroffen werden muessen.

Nachteile sind jedoch:

* Ressourcenverschwendung, da die Leitung exklusiv reserviert wird.
...
wir die ganze Autobahn von Berlin nach Wien und senden nur unser Haus, Stein
fuer Stein, ueber diese Autobahn.
...
$s$ nach $t$ einzeln adressiert werden und einzeln an den Empfaenger gesandt
...
Verbindungsaufbau und -abbau zu sparen.

Da ein Kanal (z.B. eine Reihe von Switches) nicht mehr exklusiv fuer Sender und
Empfaenger reserviert ist, kann es auf einem Kanal also mehrere moegliche
Empfaenger geben. Insofern muss fuer jede Nachricht auch irgendwie klargestellt
werden, an welchen von diesen Empfaengern sie denn nun gehen soll. Wir muessen
die Nachricht also *adressieren*. Dafuer wird der $L$-bittigen Nachricht ein
Header der Laenge $L_H$ vorangestellt, welcher exakt diese Adressinformationen
beinhaltet.
...
naechsten Knoten weitergesandt werden kann. Das bedeutet dann also, dass wir
...
auch noch der Header mitgesandt werden muss! Da wir aber nur die Nachricht
senden, und keine Signalisierungsnachrichten, haben wir die
Ausbreitungsverzoegerung $t_p$ nur einmal. Das ergibt dann fuer die
Uebertragungszeit $T_{NV}$ bei der Nachrichtenvermittlung:
...
dann nacheinander weitersenden wird.

Somit sind die Vorteile von Nachrichtenvermittlung so zusammenzufassen:

* Flexibles Zeitmultiplexing von Nachrichten,
* Bessere Ausnutzung der Kanalkapazitaet,
...
* Nachrichten muessen fuer TDM moeglicherweise bei den Vermittlungsknoten
  gepuffert werden.
* Sind die Puffer der Vermittlungsknoten in ihrere Kapazitaet beschraenkt,
...
durch *Paketvermittlung* entgegenwirken. Hierbei werden Nachrichten in kleine
Stuecke, *Pakete* genannt, aufgeteilt. Jedes Paket erhaelt dann einen eigenen
Haeder der Laenge $L_H$ und kann einzeln, unabhaengig von den anderen an den
Empfaenger gesendet werdet. Dieser Header enthaelt dabei nicht nur
...
Nachricht. In diesem Szenario gehen wir davon aus, dass alle Pakete die selbe
Groesse $p_{max}$ haben, sodass $L \mod p_{max} = 0$ ($L$ ist ein ganzzahliges
Vielfaches von $p_{max}$). Das essentielle hierbei ist, dass sobald das erste
Paket der Nachricht nach $p_{max}/r$ Sekunden serialisiert und versandt wurde,
dieses beim naechsten Knoten im Pfad auch *sofort* weitergesandt werden
kann. D.h. dass wenn eine Nachricht in 1000 Paketen gesendet wird, wir zwischen
dem Quellknoten $s$ und dem ersten Vermittlungsknoten $i$ lediglich eine
Verzoegerung von einem Paket haben, bevor $i$ weitersenden kann. Bei
Nachrichtenvermittlung haetten wir naemlich bei Knoten $i$ auch noch auf die
anderen 999 Pakete warten muessen, bevor wier die 1000 Pakete (bzw. die
Nachricht) weitersenden haetten koennen. Dass waeren fuer je zwei Knoten also
999 mal mehr Paketserialisierungszeiten. So haben wir zwischen je zwei Knoten
lediglich eine Paketserialisierungszeit, die wir warten muessen. Danach werden
Pakete naemlich auch laufend weiter gesendet. Waehrend Knoten $i$ naemlich das
erste Pakete fuer $i + 1$ serialisiert, kommt bei $i$ schon das naechste Paket
von $s$ an. Das heisst dann, dass wir fuer $n$ Vermittlungsknoten $n$ mal die
extra Paketserialisierungszeit zahlen muessen. Dazu dann noch $L/p_{max}$ mal
diese Zeit, fuer die erste Serialisierung beim Quellknoten sowie letztlich
natuerlich noch die Ausbreitungsverzoegerung $t_p$. Insgesamt also:

$$T_{PV} = n \cdot \frac{p_{max} + L_H}{r} + (\left\rceil\frac{L}{p_{max}} \cdot
L_H + L\right\rceil) + \frac{d}{\nu c}$$

Durch Paketvermittlung koennen wir also nun nicht nur Nachrichten multiplexen,
sondern Pakete. Das hat den Vorteil, dass die Aufspaltung der Zeit noch
flexibler sein kann und dass wenn ein Paket verloren geht, auch nur dieses
erneut gesendet werden muss. Auch ist die Pufferung kleiner Pakete einfacher
also die Pufferung ganzer Nachrichten.
...
Paketvermittlung wird in den meisten modernen Datennetzen verwendet.
...
skalierbare Adressierungsmoeglichkeit. Was wir moechten ist eine global
eindeutige Identifizierung der Hosts auf Port 1, sowie dann von Hosts innerhalb
dieses Sub-Netzwerks.
...
die Organisation von Rechnern weltweit viel einfacher macht.

### IPv4

Bei IPv4 geben wir jedem Host auf der Welt eine eindeutige *IP-Adresse*. Eine
*IP-Adresse* ist dabei eine 32-Bit lange Zahl, die aus 4 Bytes oder *Oktetten*
besteht. Diese Oktette trennt man zur Veranschaulichung mit einem Strich, wobei
man die Byte-Werte selbst in Dezimalschreibweise angibt. Das nennt man dann
...
beispielsweise die ersten drei Bytes, also `192.168.1` das Netzwerk
identifizieren und das letzte Oktett, also `.1`, den Host innerhalb des
Netzwerkes `192.168.1`.

Um nun eine Nachricht an einen Host mit einer bestimmten IP-Adresse zu senden,
muss man diese Nachricht an diesen Host mit seiner IP-Adresse adressieren. Der
Nachricht wird also wiederum ein Header vorangestellt, also der *IP-Header*. Der
IP-Header sowie seine Payload, also der PDU der hoeheren Schicht (Schicht 4),
bilden also die Payload eines Schicht 2 Pakets, z.B. einem Ethernet Paket.
...
2. Bits 4-7: *IHL* (*IP-Header-Length*). Dieses Feld gibt in 4 Bit die Laenge des
   IP-Headers in Vielfachen von 32 Bit bzw. 4 Byte an. Das ist notwendig, das
   IP-Header noch optionale Felder enthalten koennen, dessen Anzahl variabel
   ist. Ein IP-Header hat somit auch eine maximale Laenge, naemlich $15 \cdot 4
   = 60$ Byte bzw. 480 Bit.
...
4. Bits 16-31: Gibt die Laenge des IP-Pakets, also Header plus Daten, in Bytes
   ...
   wuerde sie *fragmentiert* werden, also in kleinere Nachrichten aufgespalten.
5. Bits 32-47: *Identification*. Ein fuer jedes IP-Paket zufaellig gewaehlter 16
   Bit Wert. Dieses wird genutzt, um fragmentierte Teile eines Pakets beim
...
6. Bits 48-50: *Flags*. Diese drei Bits stehen fuer drei Flags, naemlich:
   ...
   2. Bit 49: *Don't Fragment* (*DF*) Bit. Ist diese Flag gesetzt, darf ein
      Paket nicht fragmentiert werden.
   3. Bit 50: *More Fragments* (*MF*) Bit. Dieses Bit signalisiert, ob mehr
      ...
      fragmentiert wurde) Fragment ist.
7. Bits 51-66: *Fragment Offset*. Dieser 16-Bit Wert gibt die absolute Position
   der Daten in diesem Fragment bezogen auf das unfragmentierte Paket in
...
8. Bits 67-74: *TTL* (*Time-To-Live*). Diese Zahl gibt an, wie viele Hops ein
   Paket maximal machen soll. Ein *Hop* ist dabei eine Kante im
   Netzwerkgraphen. Erhaelt ein Router ein IP-Paket, dekrementiert er als Erstes
   den TTL um 1. Ist der TTL dann 0, so verwirft er das Paket und sendet eine
   ICMP Time-Exceeded Nachricht an den Absender. Bleibt es danach positiv,
   sendet er es weiter. Ein TTL von 0 wuerde also bedeutet, dass das Paket nur
   an den selben Host (sich selbst) gesendet werden darf. Ein TTL von 1
   bedeutet, dass das Paket nur an Hosts im selben Direktnetzwerk (Subnetz)
   gesandt werden darf, also insbesondere nie an einen Router kommen sollte. Ein
   TTL ist naemlich gleich einem Hop, also keinem Router dazwischen (nur
   z.B. einem odere mehreren Switches). Durch die TTL wird sichergestellt, das
   Pakete nicht endlos durch das Internet kreisen koennen.
9. Bits 75-82: *Protcol*. Identifiziert das Protkoll auf Schicht 4, das fuer die
   Daten in der Payload genutzt wird. Gueltige Werte sind beispielsweise TCP
   (0x06) und UDP (0x08).
10. Bits 83-98: *Header Checksum*. Eine einfache, schnelle Pruefsumme, nur fuer
    den IP-Header (also nicht den Daten). Die Pruefsumme ist konzipiert, dass
    eine Dekrementierung des TTL, welche ein Router ja durchfuehren muss, einer
    Inkrementierung desr Pruefsumme entspricht. Somit muss also nicht die ganze
    Pruefsumme neu berechnet werden. Wie bei CRC ist nur Fehler*erkennung*,
    nicht Fehler*korrektur* moeglich.
...
Der 16. Bit eines IP-Header ist reserviert und auf 0 gesetzt. Reservierte Bits
...
diesen Bit dann z.B. zufaellig auf Eins gesetzt (oder was halt in Memory
lag). Kriegt der Bit dann ploetzlich eine Bedeutung, ist es schlecht, wenn der
Bit bei manchen eben schon (unwissentlich) gesetzt wird.
...
IP-Adresse das Netzwerk identifizieren, und wieviele den Host. Daher muessen wir
also nuer pruefen, ob der Netzwerkteil der Ziel-IP-Adresse gleich dem
Netzwerkteil unserer eigenen IP-Adresse ist. Je nach Resultat, gehen wir anders vor.
...
herausfinden. Dazu koennen wir das *Adress-Resolution-Protocol* (*ARP*)
verwenden. Dabei broadcastet der Quellhost, der die MAC-Adresse seines Ziels
wissen moechte, einen *ARP-Request* an sein lokales Netzwerk. Er sendet also
einen passenden ARP-Request an die MAC-Broadcast-Adresse
`ff:ff:ff:ff:ff:ff`. Jeder Switch, der diesee MAC-Adresse im Ethernet Header
sieht, leitet das Paket also an alle seine Ports weiter, sodass auch jeder Host
im Netzwerk am Ende diesen Request bekommt. Liest der Zielhost dieses Paket und
erkennt, dass nach seiner MAC-Adresse gefragt wird, so sendet er einen
*ARP-Reply* als MAC-Unicast zurueck an den Quellhost (der ARP-Request beinhaltet
auch die Quell-MAC-Adresse). Der Quellhost erhaelt somit also die
Ziel-MAC-Adresse und kann seine Nachricht danach in einem neuen Ethernet-Paket
dorthin senden.

Der genaue Aufbau eines ARP-Pakets (welcher also in der Payload eines Ethernet
Pakets gesendet wuerd), ist wie folgt:
...
6. *Sender Hardware Address [0:31]*: Obere 32 Bit der Sender Hardware Adresse.
7. *Sender Hardware Address [32:47]*: Untere 16 Bit der Sender Hardware Adresse.
...
bis auf die Target MAC Adresse (welche er ja wissen moechte), ausfuellen. Der
Request wird dann eben an alle Hosts gebroadcastet. Der Ziel-Host sendet dann,
nachdem er diesen Request erhalten hat, einen Reply zurueck. Dabei vertauscht er
die Inhalte fuer Sender und Target, und fuellt zusaetzlich noch das vorhin leere
Target (nach Vertauschen Sender) Hardware Address Feld aus. Die Operation wird
dann noch auf Reply (0x0002) gesetzt und via Unicast (unterstes Bit des ersten
Oktetts der Target MAC-Adresse im Ethernet Header) an den Absender
zurueckgeschickt.
...
`arp -a` Befehl angezeigt werden kann.
...
replies*) oft auch im ARP-Cache gespeichert.

##### Anderes Netzwerk

Wenn ein Host eine Nachricht an einen anderen Host im selben Netzwerk senden
will, und nur desen IP-Adresse kennt, macht er also einen ARP-Request. Was ist
...
werd. Der Router wird naemlich die Target Adresse des an ihn gerichteten
...
natuerlich __nicht__ veraendert.
...
3. *Checksum*: Eine Checksum fuer die ICMP Nachricht.
4. 32 Bit die vom Type der ICMP Nachricht abhaengen.
...
3. Fuer jeden TTL Wert kommt das Paket maximal genau so viele Hops weiter. Wird
   ...
   vieln Hops also dieser Router kommt.
...
1. Ein Netzwerkadministrator verteilt die Adressen statisch, per Hand (aus dem
   Pool von Adressen den ihn sein ISP gegeben hat)
...
1. Er broadcastet einen *DHCP-Discover* Rahmen (Schicht 2), um einen DHCP-Server
   zu finden.
2. Ein DHCP-Server erhaelt diesen Rahmen und sendet einen *DHCP-Offer* mit einer
   moeglichen IP-Adresse zurueck an den Client.
...
4. Der DHCP-Server antwortet dann mit einem DHCP-ACK, um zu signalisieren, dass
   er die angeforderte IP-Adresse fuer den Client freigibt, oder einen
   DHCP-NACK, wenn er die Adresse (doch) nicht hergeben will.
...
Zeitraums muss ein Client also vom DHCP-Server eine neue Adresse anfordern. Eine
weitere interessante Eigenschaft bei DHCP ist es, dass, besonders in kleinen
Netzwerken, oftmals *Router* die Aufgaben eines DHCP-Servers uebernehmen.
...
Ein DHCP-Paket hat immer dieselbe Struktur. Interessant dabei sind drei Felder:

* *Client-IP-Address*: Falls der Client noch keine IP-Adresse hat, dann sind hier nur
  Nullen. Wenn er schon eine hat, kann er sie hier reinschreiben, z.B. fuer
  Renewing (er wuerde fuer ein Renewal einen Request zu seinem DHCP-Server
  direkt unicasten, nicht zuerst einen Discover broadcasten.)
* *Your-IP-Address*: Wenn das DHCP-Paket ein Offer oder ein ACK ist, kommt hier
  die geleased IP-Adresse rein.
...
identifizieren. Hierfuer hat man sich das Konzept von *Subnetzen* (*Classless
Routing*) ausgedacht.

Ein Subnetz ist im Prinzip einfach nur ein Netzwerk. So erhaelt nun jedes
Interface (Netzwerkkarte) nicht nur eine IP-Adresse, sondern auch eine 32-Bit
lange *Subnetzmaske*. Eine Subnetzmaske ist eine logische Maske, welche eine
Adresse in einen Netzanteil und einen Hostanteil teilt (so wie man es vorher
durch die obersten Bits eines Oktetts beim classful routing gemacht
haette). Eine logische 1 an einer Bitstelle sagt dabei aus, dass diese Bitstelle
in einer IP-Adresse zum Netzanteil gehoert, und eine 0 dass diese Stelle im
Hostanteil steht. Diese logischen Einsen muessen dabei aneinander grenzend
(contiguous) sein (man kann also Netzanteil und Hostanteil nicht mehrmals
mischen).
...
die *Broadcastadresse* des Subnetzes. Fuer das obige Beispeilsubnetz waere die
Broadcastadresse:
...
zusammenzufassen. Dieses Prinzip nennt man *Supernetting*. Man kann zwei
Subnetze aber nur dann supernetten, wenn sie sich *nur im letzen Netzwerkbit*
unterscheiden. Dann kann man diese beiden Subnetze mit $N$ Netzwerkbits naemlich
eindeutig durch ihr uebergeordnetes Netz mit $N - 1$ Bits identifizieren.

Nehmen wir beispielsweise diese beien Subnetzadressen:
...
```
11000000.10101000.00000000.00000000 / 23
```

Da der ISP uns die beiden Subnetzraeume gegeben hat, hat er uns ja im Prinzip
dieses Supernet gegeben. Also gehoert dieses /23 Supernet auch sicher uns.
...
* 0.0.0.0/8: *Hosts in diesem Netzwerk* (oder "default"-Adresse)
...
* 127.0.0.0/8: *Loopback-Adressen*
  - Identifiziert den lokalen Rechner (__localhost__): 127.0.0.1 (127.0.0.0 ist
    die Netzadresse, keien Host-Adresse!)
  - Wird nicht geroutet.
...
1. Oktette 0 bis 5 (48 Bit): Praefix
...
1. Bits 0-3: *Version*. Gibt wie bei IPv4 Headern die IP Version an (dieses Feld
   ist das Einzige, welches bei IPv4 und IPv6 gleich ist). Gueltige Werte sind 4
   oder 6.
...
3. Bits 12-31: *Flow Label*. Ein Identifikator fuer einen *Flow*. Das war
   ...
   gleich behandelt werden sollen. D.h. sie sollen ueber den selben Pfad gesandt
   werden.
...
1. *Next Header*: Den Typ des naechsten (inneren) Header.
...
also wissen, was die MTU auf dem Weg von sich selbst bis zum Sender ist.
...
* fe80::/10 - *Link-Local Adressen*
  ...
  - Sie wird aus dem Interface-Identifier generiert.
...
3. Multicast
   ...
   * Passiert mittels spezieller Multicast-Adressen.
...
4. Anycast
   * Pakete, die an eine beliebige Station einer bestimmten Gruppe adressiert
     sind.
   * Zum Beispiel alle Nameserver.

Wie entscheidet ein Host aber, ob ein Multicast Paket an ihn adressiert ist? Die
Antwort ist, dass es bestimmte *Multicast Grupppen* gibt. Jeder Host ist immer
in zumindest einer, aber moeglicherweise auch vielen Multicast Gruppen. Erhaelt
ein Host ein IPv6 Multicast Paket, muss er nur pruefen, ob die Multicast Gruppe,
an welche das Paket adressiert ist (die IPv6 Adresse des Pakets) eine der
Gruppen ist, in welcher sich der Host befindet. Ist der Host nicht in dieser
Gruppe, braucht er es auch gar nicht weiter zu bearbeiten (z.B. auf L3). Die
folgenden Adressen sind Beispiele fuer Multicast-Gruppen:

* ff02::1 - *All Nodes*
  - Adressiert alle Knoten auf dem lokalen Link.
  - Man bemerke: Es gibt in IPv6 keine explizite Broadcast Adresse (insbesondere
    ist die hoechste Adresse in einem Subnetz eine valide IPv6 Adresse!),
    sondern nur diese spezielle Art von Multicast.
...
eingehende Multicast Nachrichten entsprechend filtern. Wird ein Router nun Teil
einer neuen Multicast Gruppe, z.B. wenn er zum ersten Mal eine IPv6 Adresse
erhaelt und so automatisch Teil der *All Nodes* Multicast Gruppe wird , so
informiert er den Router dabei durch einen *MLD Report* (`exlude Host A from the
filter for this multicast group`). Das selbe tut er auch, wenn er nicht mehr
Teil der Gruppe wird. MLD ist uebrigens Teil von ICMPv6.

Zusaetzlich dazu gibt es dann noch das Konzept des *MLD Snooping*. Hierbei
hoeren Switches im Netzwerk die MLD Nachrichten ab, um zu erfahren, auf welchen
Ports bestimmte Multicast Gruppenmitglieder present oder nicht sind. Ein Switch
muss dann Multicast Pakete nur an jene Ports sendne, wo der Switch weiss, dass
es dort einen Host in dieser Gruppe gibt. Fuer *All Nodes* waere das natuerlich
jeder Port. Da bei MLD immer gemeldet wird, wenn ein Host Teil einer neuen
Gruppe wird, kann der Switch aber auch fuer andere Gruppen immer genau wissen,
ob auf einem Port Mitglieder dieser Gruppe sind, oder nicht.

#### Solicited Node Address

Es seien noch einige Details zu *Solicited Node Adressen* (*SNA*) angemerkt. Zuerst
wollen wir uns ansehen, wann und wie eine solche Adresse generiert
wird. Grundsaetzlich erhaelt eine Host fuer jegliche IPv6 Adresse, egal ob
link-local oder global, immer eine entsprechende Solicited Node Address. Geben
wir einem Host beispielsweise die globale IPv6 Adresse
...
Bit) der urspruenglichen IP Adresse an den 104-Bit Praefix `ff02::1:ff00:0`
hinten anhaengt. Also:
...
Adresse ist. Sie identifiziert also eine *Multicast* Gruppe. Sobald ein Host
also eine IP-Adresse bekommt, erhaelt er auch diese SNA und faellt in dies
Multicast Gruppe.

Jetzt kommt das allerwichtigste: Bei Neighbour Discovery will ein Host nun also
...
Identifier gleich sind. Z.B. koennte (A) ein Computer von Apple sein und (B) ein
Router von Cisco, sodass ihre oberen 24 Bit (OUI) verschieden waeren. Nun bilden
wir via SLAAC ihre link-local IPv6 Adressen:
...
Jetzt haben wir also unsere link-local Adressen IPv6 erforder nun die
...
also die SNA Gruppen, indem wir die unteren 24 Bit an den Praefix
`ff02::1:ff00:0/104` knuepfen:
...
lokalen Link. Sendet ein Host also ein Neighbor Discovery Paket an diese SNA,
werden diese beiden Hosts es sicher bekommen und verarbeiten. Sie muessen dann
also auch als einzige Hosts (unter der Annahme dass es nicht noch mehr Hosts in
dieser SNA Gruppe gibt) das Neighbour Discovery Paket, also den Neighbour
Soliciation Request, inspizieren. In diesem steht dann die genaue 128-Bit Target
Adresse drin, welche natuerlich nur entweder (A) oder (B) gehoeren wird.
...
aber weiss er, was die Multicastgruppe ist? Wir haben oben ja verschiedene
...
man zu dem `33:33` Praefix der Multicast MAC-Adresse noch die unteren vier
...
wuerde man diese and die `ff02::2` IPv6 Multicast Adresse senden wollen -- auf
Schicht 3. Auf Schicht 2 wuerde man die entsprechend MAC-Multicast Adresse dann
so bilden:

1. Man nimmt den `33:33` Prefix fuer IPv6 Multicast MAC Adressen.
2. Und gibt die unteren 32 Bit (4 Oktette) der IPv6 Multicast Adresse hinzu.
...
*stateless address autoconfiguration* (SLAAC; stateless, weil diese Adressen
nicht von einem DHCP-Server vergeben werden). Dies Adresse sieht dabei wie folgt
aus:
...
3. Danach kommt die MAC Adresse ins Spiel:
   1. Zuerst kommen die 24 Bit des OUI (Organizationally Unique
      Identifier).
   2. Dann kommen 16 Bit Padding, wobei dieser Raum mit `ff:fe`.
...
administered link-local Adresse zu signalisieren (die Adressen waeren dann
weniger schoen). Eine solche vergebene MAC Adresse waere natuerlich locally
administered, weil sie ja manuell (lokal) vergeben wird.
...
2. Bits 8-15: *Code*. Praezisiert den Typ der Nachricht, z.B. ob es sich um
   einen Echo *Request* oder Echo *Reply* handelt, wenn der Typ *Echo* ist.
...
Schicht 1), muss aber Router nicht unnoetig belasten. Wuerde man in diesen
Pseudo Header beispielsweise noch den Hop Limit einfuegen, so muesste man
natuerlich nach jedem Hop die Checksum neu berechnen.
...
4. Router Advertisement (Type 134)
...
* *Duplicate Address Detection*: Ein Knoten kann ueberpruefen, ob seine IPv6
  Adresse im Link schon benutzt wird (wie mit ARP bei IPv4).
* *Parameter Discovery*: Hosts koennen Parameter fuer einen Pfad finden,
  ...
  gewaehlte Interface eines Routers groesser ist als ein IPv6 Paket, verwirft
  dieser das Paket und sendet eine ICMPv6 *Packet Too Big* (Type 2) Nachricht
  zuruck an den Quellhost. Was ein Host bei IPv6 also macht, ist anfangs
  anzunehmen, dass die MTU fuer den gesamten Pfad gleich der MTU auf dem lokalen
  Link ist, welche es eben durch diese Parameter Discovery herausfinden
  kann. Kommt das Paket dann zu einem Router auf diesem Pfad, dessen MTU kleiner
  ist als das Paket, sendet es eben eine solche *Packet Too Big*-Nachricht
  zurueck an den Host. In dieser ist dabei auch die MTU dieses Routers
  enthalten. Der Quellhost kann sein Paket dann also entsprechend dieser MTU
  fragmentieren und probiert dann die Fragmente nochmals wegzusenden. Das macht
  er so lange, bis die Fragmente klein genug sind.
...
seine MAC Adresse, um ihm eine Nachricht zu senden.

Als Erstes wuerde dieser Host, der die MAC Adresse seines *Neighbors* finden
will, einen *Neighbor Solicitation* Request. Dieser Neighbor Soliciation Request
...
3. Die 128 Bit IPv6 Adresse des Interface, dessen MAC Adresse man finden moechte.
...
drin) gemulticastet werden. Wenn Switches kein MLD Snooping machen und nicht
wissen, an welchen Ports welche Gruppenmitglieder sind, artet das dann auch
...
1. Pruefen, ob er Mitglied der Solicited Node Multicast Gruppe ist, an welche
   ...
   gerade alle Hosts die in diser SNA Gruppe sicher drin sind. Ansonsten sind
   das alle Hosts im lokalen Link.
...
3. Wenn er schon drin ist, entpackt er das IPv6 Paket um an die ICMPv6 Nachricht
   bzw. die Neighbour Soliciation in der Payload des Pakets zu kommen. Dann
   prueft er genau ob die gewuenschte 128 Bit Adresse ihm gehoert, oder nicht.

In Retour kommt vom passenden Host dann ein *Neighbour Advertisement*
...
Ein Router erhaelt nun also ein Paket. Was routet er es? Er geht so vor:
...
4. Das Paket wird dann an den Next Hop (Gateway) gesendet. Ist dieser 0.0.0.0,
   ...
   lokalen Link. Ist der Gateway nicht diese Default-Adresse, sendet er es also
   an den Next Hop, welcher ebenso ein Router ist (und bei sich fuer seine
...
Nehmen wir ein Beispiel. Der Router mit der obigen Tabelle erhaelt nun ein
...
1. Distanz-Vektor Protokolle:
   ...
   * Router tauschen unterinander lediglich __akkumulierte Kosten__ aus.
...
inkrementieren den Hop Count einfach um einen Hop. Bei Link-State Protkollen
...
Nachbarschaftsbeziehungen (Kanten und Kosten). Kennt der Knoten erstmal die
gesamte Netztopologie, rechnet er sich *selbst* den kuerzesten Pfad durch das
*gesamte Netzwerk* zu jedem Ziel aus. Deswegen kann man Link-State Protokolle
auch nicht verteilt implementieren, da ja jeder Knoten das gesamte Netzwerk
kennen moechte (beim Dijkstra Algorithmus: deswegen hat jeder Knoten einen Heap
fuer das gesamte Netzwerk).
...
* Nun haben wir noch eine Priority Queue (Heap), in welchem immer der billigste
  ...
  sodass (Knoten, Kosten)-Paare gespeichert werden koennen. Diess Paar soll
...
Router Routing Tabellen austauschen zu lassen. Es hat nur ein einzige Metrik
fuer Verbindungen: den Hop Count. Das waere also so, als ob man allen Kanten im
Graphen die selben Kosten geben wuerde. Diese eine Metrik ist natuerlich nicht
sehr realistisch, da beispielsweise eine optische Leitung viel schneller ist als
eine alte Kupferleitung.
...
einen Eintrag fuer $A$. Wenn $B$ nun also den Hop Count fuer alle seine
Eintraege inkrementiert und $C$ eine Route fuer $A$ mit Kosten $2$ sendet,
...
Betrachten wir fuer dieses Problem das folgende Netzwerk (mit eingezeichneten Kantenkosten):
...
$B$ seine Routing Tabelle nach dem RIP Protokoll broadcastet (da die Reihenfolge
...
wird deswegen auch *Count to Infinity* genannt (Anmerkung: Es ist aber nur ein
fancy Name fuer einen Routing Loop).
...
1. Split Horizon
   * "Sende dem Nachbarn, von dem du die Route zu $X$ gelernt hast, keine Route
     zu $X$"
   * $A$ wuerde nach dem ersten Update von $B$ also dem Knoten $B$ (dem Gateway
     von $A$ fuer $C$) keine Route zu $C$ senden.
...
2. Poison Reverse
   ...
   * Aehnlicher Ansatz wie Split Horizon.
...
3. Path Vector
   ...
   * Ein Router prueft dann vor Installation einer Route in seine Tabelle, ob er
     wohl nicht selbst in dieser Route drin ist.
...
Schwaechen hat. Wir wollen uns noch andere Beispiele fuer echte Protokolle
ansehen:
...
* Link-State Protokolle
  ...
  - *IS-IS* (*Intermediate System to Intermediate System*): Seltener
    ...
    da es sein eigenes L3-Protokoll mitbringt.
...
Netzwerken, die unter einheitlicher adminstrativer Kontrolle stehen (z.B. durch
die deutsche Telekom). Ein AS wird durch einen 16 Bit Identifier, der
sogenannten AS-Nummer, identifiziert.
...
AT&T. Auf Tier 2 sind kleinere AS und auf Tier 3 wiederum.
...
1. *Peering Verbindungen*: Vereinbarungen zwischen AS, Traffic untereinander zum
   gegenseitigen Nutzen beider Systeme kostenlos auszutauschen.
...
Die Border-Router eines AS (welche also ein EGP einsetzen wuerden) announcen
Praefixe an seine Peerings und Upstream-Provider. Wenn ein Tier 2 AS mit seinem
Traffic nicht weiter weiss, leitet er es an seinen Default Gateway weiter,
welcher ein Router eines Upstream-Providers sein wird. Kommt es in eine Tier 1
AS und kann auch dort nicht verarbeitet werden, so wird das Paket
verworfen. Tier 1 AS muessen naemlich *immer* wissen, wo sie Traffic hinsenden
koennen, da sie keine Default Gateways mehr haben (welche bei einem niedrigeren
Tier eben Router in hoeheren Tiers waeren). Deswegen nennt man Tier 1 auch die
*Default Free Zone*.
