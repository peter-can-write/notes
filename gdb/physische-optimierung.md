...
$\sigma_{p_1}(R) \theta \sigma_{p_2}(S)$, wo $\theta$ eine passende Operation
ist. Hier gibt es nun zwei moegliche Datenflussvarianten:

1. Selektiere alle gueltigen Tupel aus $R$.
2. Selektiere erst dann alle gueltigen Tupel aus $S$.
...
* sort
* Duplikateliminierung: unique/distinct
...
Selektionspraedikat fuer dieses Element zu testen. Ein soclher Iterator hat
...
Bedingung erfuellt, damit `next()` folglich in konstant Zeit laufen kann.
...
`iterator nested_loop_p`
	...
	* next:
	  ...
	  + Sollte die rechte Eingabe ohne passenden Partner erschoepfen, rufe
        `next` erneut auf.
	  + Findet sich ein passendes Paar, fuege es zum Resultat hinzu.
	* close: Schliesse beide Eingabequellen.

```python result = [] left.open() for i in left: if right.is_closed():
right.open() for j in right: if predicate(i, j): result.append((i, j))
...
bestehen aus $m$ Seiten in zwei Mengen $X, Y$ zu partitionieren, wobei also
...
$S$. Wurde die letzte Teilmenge $Y$ geholt und diese mit $X$ vollstaendig
...
N)$). Danach werden die beiden Relationen parallel und sequentiell
abgestiegen. Seien $R, S$ nun zwei Relationen, fuer welche $R \bowtie_{R.A=S.B}$
durchgefuehrt werden soll, wo also $A$ ein Attribut von $R$ ist und $B$ ein
...
werden aber nur einige Elemente/Tupel gleich sein. Dabei muss man jedoch achten,
dass man zum ersten Element einer Aequivalentsklasse (bzw. hier einer
...
Element $\in S$ in einer Gleichheitsklasse.
...
Eine Alternative ist es, nur eine Relation zu sortieren und auf die andere
einen Index zu geben:
...
Seiten fein genug sind und (auf zufriedenstellende Weise) in den Hauptspeicher
...
Schluessel auf einen Index (fuer eine Partition) abbildet. Weiters wird das
Verfahren so angeordnet, dass von $m$ freien Hauptspeicherseiten $m - 1$ als
temporaere Partitionen verwendet werden, und eine als Inputseite. Der Sinn der
temporaeren Partition ist, erst wenn die temporaere Partition voll ist die
Zugriffsluecke zum Hauptspeicher zu ueberqueren und die Daten in den
endgueltigen Partitionen zu hinterlegen.
...
gegeben. $h_1(x)$ gibt dann fuer den Schluessel $x$ den Partitionsindex
an. Folglich wird das Element in eine der $m - 1$ temporaeren Partition im
Hauptspeicher gegeben. Ist eine der $m - 1$ temporaeren Partitionen voll, so
muss die Zugriffsluecke also ueberwunden werden und die Daten werden in die
endgueltige Partition uebergeben. Am Ende werden dann auch zu dem Zeitpunkt
nicht volle temporaere Partitionen ausgelagert.

Sind die Partitionen nun noch nicht fein genug, "rekursiert" man fuer jede der
vorhin kreeirten Partitionen auf selbe Weise. Man laedt die Partition
seitenweise in die Inputseite (eine Partition kann ja -- zu dem Zeitpunkt noch
-- mehrere Seiten im Hintergrundspeicher belegen), und wendet nun __die
Hashfunktion $h_2$ an__, um die Partition auf wiederum $m - 1$ kleinere Buckets
zu hashen.

Nach einer gewissen Anzahl an Iterationen terminiert man also. In den einzelnen
Partitionen sind nun (je nach Parametern natuerlich) noch relativ viele
verschieden Elemente, da die Tupel ja nur auf $m - 1$ Klassen aufgeteilt
wurden. Hat man z.B. die Schluessel-Multimenge $[300]$ (sei das eine Multimenge
mit mindestens den Zahlen $1...300$, die aber noch Duplikate der Zahlen
enthalten kann) und die Hashfunktion $h(x) = x/100$ wo $m - 1 = 3$, dann sind in
den drei Partitionen jeweils noch mindestens $100$ Elemente enthalten. Erst in
der endgueltigen Hashtabelle, in der die Partitionen im letzten Schritt dann
geladen werden, wird eine feinere Hashfunktion, z.B. $h(x) = x/100$ verwendet.
...
Iterationsanzahl. Das Resultat sind also Paare von Partitionen wobei eine
Partition eben zum Probe- und eine zum Build-Input gehoeren. Eine Partition ist
...
gewissen Anzahl an Partitionen (jeweils Seiten) aus dem Build-Input in
Hashtabellen mit der letzen Hasfunktion $h_n$ im Hauptspeicher organisiert. Dann
wird die gleiche Anzahl an Partitionen aus dem Probe-Input geladen. Fuer jedes
Paar an Partitionen geht man nun also durch jedes Element im Probe-Input durch
...
Probe-Input gegen den Hash-Table seiner Partition hashen und den erste (noch
...
und gibt beim naechsten Call zu `next` das Attribut des momentanen Tupels
wieder. Bei der Vereinigung (da nicht auf Duplikateliminierung geachtet werden
muss), gibt man einfach zuerst die Tupel der einen Menge und dann der anderen aus.
...
Hier ist der Ausdruck $X$ also mehrmals enthalten. Man kann ihn also nur einmal
auswerten, in einen Bucket speichern, und mehrmals verwenden, dann sieht der
Graph so aus:
...
Stuecke -- hier sogenante *Laeufe* ("runs") -- aufzuteilen, und diese dann mit
einem *Merge*-Verfahren rekursiv in ein einziges sortiertes Stueck zu
...
3. Nehme das naechste Element in der Eingabe Queue (es werden also
   *seitenweise* neue Elemente aus dem Plattenspeicher geholt).
   ...
   Elemente, dann gib es ans Ende der Heap oder in eine andere Liste -- "sperre"
   es jedenfalls fuer den restlichen Lauf.
...
entsanden waere, haette man nach dem Sortieren sofort die Elemente als fertigen
