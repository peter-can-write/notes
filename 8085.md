...
entahelt also nur das Leitwerk und das Rechenwerk sowie Busse bzw. Verbindungen
...
* Drei 16-Bit Register:
  + Stack-Pointer
  + Befehlszaehler
  + Auf-und-Abwartszaehler zum Verarbeiten von Strings/Datenbloecken. Mit diesen
...
hat einen Eigang um einen Interrupt anzufordern (interrupt request; INTR), einen
...
zwei separate Signale). Dies Redundanz (eine Bit wuerde ja genuegen) ist zum
...
Der Maschinenbefehlszyklus des 8085 besteht in vereinfachter aus den folgenden
vier Phasen:
...
* __$1$ bis $5$ Maschinenbefehlszyklus__
...
__Instruktionen zu fetchen dauert je nach Instruktionslaenge $4$ bis $6$
...
es zwischen dem Ein-/Ausgabewerk und dem Prozessor einen Puffer (ein Register),
in welchem Daten vom Prozessor abgeholt (fuer input) oder eingeschrieben (fuer
output) werden koenen. Anstelle nun separate Adressen fuer diesen Puffer zu
...
werden, benutzt man fuer den E/A Puffer nur $8$ der moeglichen $16$ Bit.
...
Er dauert drei Maschinenzyklen und drei Taktzyklen, falls der Speicher nicht
...
2. Maschinenzyklus: Ausgabeport aus dem Hauptspeicher laden (ist eine
   Konstante); 3 Takte.
3. Maschinenzyklen: IO/!M geht nun auf IO und die Ausgabeportadresse geht an das
...
2. Implizit: Fuer Befehle, bei welchen die Adressen implizit sind. Z.B. XCHG,
...
5. Registerindirekt: Der Operand des Befehls ist ein Register, das die
   Speicheradresse des eigentlichen Operandne enthaelt (`inc [eax]`)
